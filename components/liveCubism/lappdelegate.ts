...
import { canvas, gl, LAppGlManager } from './lappglmanager';
...
export class LAppDelegate {
  ...
  public initialize(): boolean {
    ...
    // キャンバスを DOM に追加
    document.body.querySelector(".live2d-container")?.appendChild(canvas);
  ...
  public getView(): LAppView {
    return this._view as LAppView;
  }

  public getTextureManager(): LAppTextureManager {
    return this._textureManager as LAppTextureManager;
  }
  ...
  constructor() {
    ...
    console.log('LAppView is created');
  ...
  if (!canvas || !gl) {
    console.error('Canvas or WebGL context is not available.');
    return;
  }

  const devicePixelRatio = window.devicePixelRatio || 1; // Fallback for environments without `devicePixelRatio`
  
  // Cache the current dimensions
  const displayWidth = Math.floor(window.innerWidth * devicePixelRatio);
  const displayHeight = Math.floor(window.innerHeight * devicePixelRatio / 2);
